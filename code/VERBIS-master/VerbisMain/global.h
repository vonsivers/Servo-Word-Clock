...
struct strConfig {
  ...
  uint8_t isFormat24h;                   //1 byte
  uint8_t c_e_o;                         //1 byte
  String startColor;
  String endColor;
...
void WriteConfig(){
  ...
  EEPROM.write(192, config.isFormat24h);
  EEPROM.write(194, config.c_e_o);
  WriteStringToEEPROM(196, config.startColor);
  WriteStringToEEPROM(206, config.endColor);
...
boolean ReadConfig(){
  ...
  if (EEPROM.read(0) == 'C' && EEPROM.read(1) == 'F'  && EEPROM.read(2) == 'G' )
  {
    ...
    config.isFormat24h = EEPROM.read(192);
    config.c_e_o = EEPROM.read(194);
    config.startColor = ReadStringFromEEPROM(196);
    config.endColor = ReadStringFromEEPROM(206);
...
void printConfig(){
  ...
  Serial.printf("Device Name:%s\n", config.startColor.c_str());
  Serial.printf("Device Name:%s\n", config.endColor.c_str());
    // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
